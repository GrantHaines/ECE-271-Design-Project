...
\begin{figure}[H]
    \includegraphics[width=0.8 \linewidth]{images/vgaOutputDiagram.png}
...
\begin{figure}[H]
    \includegraphics[width=0.8 \linewidth]{images/vgaSyncCounterCompDiagram.png}
    \caption{VGA syncCounterComp module diagram}
    \label{vgaSyncCounterCompDiagram}
\end{figure}

Inputs: An input clock signal, a 50MHz clock signal, and a reset signal.
Outputs: The horizontal or vertical sync rate for the VGA output and a signal indicating that it is in the display area for the screen.

Description: The syncCounterComp creates either the hSync or vSync signal for the VGA driver, as well as indicating that the signal is within the display area so that the RGB values can be communicated to the screen. The hSync signal travels through the horizontal row of pixels on the screen, displaying color when appropriate. The vSync signal indicates when to move to the next row.

\subsubsection{VGA counter}

\begin{figure}[H]
    \includegraphics[width=0.8 \linewidth]{images/vgaCounterDiagram.png}
...
\begin{figure}[H]
    \includegraphics[width=0.8 \linewidth]{images/vgaDisplayMuxDiagram.png}
...
\begin{figure}[H]
    \includegraphics[width=0.8 \linewidth]{images/vgaComparatorDiagram.png}
...
\begin{figure}[H]
    \includegraphics[width=0.8 \linewidth]{images/vgaSynchronizer.png}
...
\begin{figure}[H]
    \includegraphics[width=0.8 \linewidth]{images/clockDivBy2Diagram.png}
...
int d7 = 7;
int d8 = 8;
int d9 = 9;				// the last (most significant) output bit

void setup() 
{
  pinMode(analogPin, INPUT);              // Potentiometer input pin
  
  pinMode(d0, OUTPUT);           //Indicates the pinmode of selected pin
  pinMode(d1, OUTPUT);           //Indicates the pinmode of selected pin
...
void loop() 
{
  ...
  for (int i=0; i<=9; i++)
  {
    pinArray[i] = i;			// assigns outputs 0-9
    if (bitRead(value,i))
